...
void __fastcall Hooks::ReduceTimersServer(void* ecx)
{
	//if (game_movement_server->player->m_flStamina > 0)
	//{
	//	game_movement_server->player->m_flStamina -= globals->frametime * 60.f;		//60.f - sv_staminarecoveryrate

	//	if (game_movement_server->player->m_flStamina < 0)
	//	{
	//		game_movement_server->player->m_flStamina = 0;
	//	}
	//}
...
int __fastcall Hooks::TryPlayerMoveServer(void* ecx, void* ebp, Vector* pFirstDest, trace_t* pFirstTrace)
{
	...
	VectorCopy(game_movement_server->mv->m_vecVelocity, original_velocity);  // Store original velocity
	VectorCopy(game_movement_server->mv->m_vecVelocity, primal_velocity);
	...
	{
		if (game_movement_server->mv->m_vecVelocity.Length() == 0.0)
			break;

		// Assume we can move all the way from the current origin to the
		//  end point.
		VectorMA(game_movement_server->mv->GetAbsOrigin(), time_left, game_movement_server->mv->m_vecVelocity, end);
		...
		if (false)
		{
			...
			else
			{
				...
				trace_t foo;
				TracePlayerBboxCSGO_server(game_movement_server->mv->GetAbsOrigin(), game_movement_server->mv->GetAbsOrigin(), MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, foo);
				if (foo.startsolid || foo.fraction != 1.0f)
				{
					Msg("bah\n");
				}
#endif
				game_movement_server->TracePlayerBBox(game_movement_server->mv->GetAbsOrigin(), end, game_movement_server->PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, pm);
			}
		}
		else
		{
		...
		if (pm.allsolid)
		{
			...
			VectorCopy(vec3_origin, game_movement_server->mv->m_vecVelocity);
			return 4;
		}
		...
		if (pm.fraction > 0)
		{
			if (numbumps > 0 && pm.fraction == 1)
			{
				...
				trace_t stuck;
				game_movement_server->TracePlayerBBox(pm.endpos, pm.endpos, game_movement_server->PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, stuck);
			...
			game_movement_server->mv->SetAbsOrigin(pm.endpos);
			VectorCopy(game_movement_server->mv->m_vecVelocity, original_velocity);
			numplanes = 0;
		}
		...
		move_helper_server->AddToTouched(pm, game_movement_server->mv->m_vecVelocity);
			...
			game_movement_server->player->GetGroundEntity() == NULL)
		{
			for (i = 0; i < numplanes; i++)
			{
				...
				{
					ClipVelocity(original_velocity, planes[i], new_velocity, 1.0 + cvar->FindVar("sv_bounce")->GetFloat() * (1 - game_movement_server->player->m_surfaceFriction()));
				}
			}

			VectorCopy(new_velocity, game_movement_server->mv->m_vecVelocity);
			VectorCopy(new_velocity, original_velocity);
		}
		else
		{
			for (i = 0; i < numplanes; i++)
			{
				ClipVelocity(original_velocity, planes[i], game_movement_server->mv->m_vecVelocity, 1);
				...
				for (j = 0; j < numplanes; j++)
				{
					if (j != i)
					{
						...
						if (game_movement_server->mv->m_vecVelocity.Dot(planes[j]) < 0)
							break;	// not ok
					}
				}
			...
			{	// go along the crease
				...
				d = dir.Dot(game_movement_server->mv->m_vecVelocity);
				VectorScale(dir, d, game_movement_server->mv->m_vecVelocity);
			}
			...
			//
			d = game_movement_server->mv->m_vecVelocity.Dot(primal_velocity);
			if (d <= 0)
			{
				//Con_DPrintf("Back\n");
				VectorCopy(vec3_origin, game_movement_server->mv->m_vecVelocity);
	...
	float fLateralStoppingAmount = primal_velocity.Length2D() - game_movement_server->mv->m_vecVelocity.Length2D();
	...
	if (fSlamVol > 0.0f)
	{
		game_movement_server->PlayerRoughLandingEffects(fSlamVol);
	}
